...
def get_zone_number(input_record, criteria_id, df_empty_bins, search_times):
    ...
    if count_empty == 0:
        ...
        if count_empty < config.MAX_EMPTY_BINS:
            # zone alphas array (ex): D -> [D, C, E, B, F, A, G, H, I]
            zone_alphas_array = change_zone_alpha(criteria_id.zone_alpha)

            # print(f"zone_alphas_array{zone_alphas_array}")

            # zone_alpha_flag = 0

            for zone_a in zone_alphas_array:
                ...
                if criteria_id.zone_num >= criteria_id.mid_zone_num:
                    ...
                    zone_alpha_flag = "MAX_TO_MIN"     # larger to smaller(descending)
                else:
                    alternate_criteria_id = criteria_id._replace(
                        zone_alpha=zone_a,
                    ...
                    zone_alpha_flag = "MIN_TO_MAX"     # smaller to larger(ascending)
...
def get_devan_height(input_record, criteria_id, df_empty_bins, search_times) -> int:
    ...
    # print(f"get_devan_height - df_limited:\n{df_limited}")
    # exit(0)
    return df_limited
...
def change_zone_number(input_record, criteria_id, df_empty_bins, search_times, zone_alpha_flag) -> int:
    ...
    if zone_alpha_flag == "MIN_TO_MAX":
    ...
    elif zone_alpha_flag == "MAX_TO_MIN":
        ...
        while True:
            ...
            next_calculated = True
            next_zone_num -= 1
            # print(f"next_zone_num:{next_zone_num}")
            if (len(df_zone_number) < 0 or
                    not next_calculated or
                    next_zone_num < 0 or
                    next_zone_num > next_bin_id.max_zone_num
        ...
        # print(f"inside change_zone_number-3")
        while True:
            next_calculated = False
            next_flag = 0
            ...
            if len(df_zone_number) > 0 or not next_calculated or phase > config.PHASE_MAX :
                # print(f"✅ Loop finished.(phase count:{phase})")
                break
    # print(f"end of change_zone_number")
